...
namespace Libarary_ManagementSystemApp
{
    public partial class IssueBooks : UserControl
    {
            ...
            if (connect.State == ConnectionState.Closed)
            {
                try
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(selectData, connect))
                    {
                        ...
                        {
                            table.Load(reader);
                            table.Rows.Add(reader["id"], reader["book_title"]);
                        }
        ...
        private void bookIssue_bookTitle_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                string selectedBookTitle = bookIssue_bookTitle.SelectedItem.ToString();
                int selectedBookID = (int)bookIssue_bookTitle.SelectedValue;

                // Use the selected book ID to retrieve the book details from the database
                string selectData = "SELECT * FROM books WHERE id = @id AND date_delete IS NULL";
                using (SqlCommand cmd = new SqlCommand(selectData, connect))
                {
                ...
                if (connect.State != ConnectionState.Open)
                {
                    ...
                    if (check == DialogResult.Yes)
                    {
                        try
                        {
                            connect.Open();
                            DateTime today = DateTime.Today;
                            string updateData = "UPDATE issues SET date_udate=@dateDelete WHERE issue_id=@issueID";
                            using (SqlCommand cmd = new SqlCommand(updateData, connect))
                            {
